...
model.fit(x_train, [y_train[:,:3],y_train[:,3:]], batch_size=batch_size,epochs=epochs,verbose=1,validation_data=(x_test, [y_test[:,:3],y_test[:,3:]]))
...
#for x in final_pred:
 #   print('final pred: ' ,x)
#print('final pred: ',final_pred[1][:,0])
...
for ii in range(num_classes):
    ...
    if(nn<=2):
        ...
        R2  = r2_score(y_test[:,nn_1],final_pred[1][:,nn_1])
        ax[ii].set_title(titles[ii])
        ax[ii].set_ylabel('the predicted values')
        ax[ii].set_xlabel('the true values')
        ax[ii].annotate('R2='+('%0.3f' % R2 )+'',xy=(0.05,0.9),xycoords=
          'axes fraction',bbox=dict(boxstyle="round",fc="w"), size=14)
        ax[ii].plot(y_test[:,nn_1], y_test[:,nn_1], 'k-') # where the trend should be
        ax[ii].plot(y_test[:,nn_1], final_pred[1][:,nn_1], 'bo', mfc='white', alpha=0.5)
        nn_1+=1 
fig.tight_layout()
